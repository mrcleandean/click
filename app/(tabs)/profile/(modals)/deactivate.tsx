...
const Deactivate = () => {
    ...
    const tryDeactivate = async () => {
        try {
            ...
            console.log('Pending deactivation functionality');
        } catch (e) {
            setShowError(true);
            console.log(e);
        }
    ...
    return (
        <View className="w-full flex-[1] flex items-center p-7" style={{ backgroundColor: theme[currentTheme].primary, gap: 20 }}>
            <Text className="text-lg" style={{ color: theme[currentTheme].highColor }}>Please enter your password to Deactivate your account</Text>
            ...
            <TouchableOpacity className="bg-[#FF3131] p-3 rounded-lg flex justify-center items-center flex-row" style={{ gap: 5 }} onPress={tryDeactivate}>
                <Text style={{
                    color: 'white'
                }}>
                    Confirm Deletion
